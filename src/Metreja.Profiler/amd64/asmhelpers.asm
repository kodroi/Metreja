...
;   SUB RSP, 120: for 32 shadow + 6*16 XMM save = 128... let's compute:
;
; Actually, for XMM saves we need 6 regs * 16 bytes = 96 bytes, plus
; 32 bytes shadow space = 128 bytes. But RSP is currently aligned to 16
; after 7 pushes + return addr. SUB 128 keeps alignment.
;
...
EnterNaked PROC
    ...
    ; Allocate space: 32 shadow + 96 XMM = 128 bytes
    sub     rsp, 128
    ...
    ; Call C++ stub - RCX and RDX still hold the original parameters
    ; (we pushed them but the CLR put them in RCX/RDX originally,
    ;  so we reload from the saved locations)
    mov     rcx, [rsp+128+40]   ; original RCX (skip 128 + 5 pushes after RCX)
    mov     rdx, [rsp+128+32]   ; original RDX (skip 128 + 4 pushes after RDX)
    call    EnterStub
